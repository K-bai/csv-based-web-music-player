<template>
  <div class="c-copy-code">
    <div v-for="(up, idx) in call_list" v-bind:key="up.name" class="copy-item">
      {{ up.code }}
      <el-popover trigger="click" v-bind:content="popper">
        <a slot="reference" v-on:click="copy(idx)" class="copy-button">复制</a>
      </el-popover>
    </div>
  </div>
</template>
