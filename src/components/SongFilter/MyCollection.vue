<template>
  <div>
    <div class="title title-filter">
      ...
      >{{show_my_collection?'...收起':'展开...'}}</div>
    </div>
    <div class="c-song-collection" v-show="show_my_collection">
      <div
        v-bind:class="['collection-item', {'collection-item-deleting': is_deleting_collection}]"
      ...
      >
        <img src="~bootstrap-icons/icons/bookmark-star.svg" v-show="!is_deleting_collection" />
        <img src="~bootstrap-icons/icons/trash-fill.svg" v-show="is_deleting_collection" />
...
export default {
  ...
  methods: {
    ...
    add_collection() {
      ...
      // 查找同名歌单
      let idx = this.my_song_collection.map(c=>c.name).findIndex(n=>(n === this.add_collection_name))
      if (idx !== -1) {
        // 已有同名就更新
