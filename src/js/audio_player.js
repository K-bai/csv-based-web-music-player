...
class MeUmyAudioPlayer {
    ...
    init_ctx_events() {
        ...
        this.ctx.addEventListener('progress', ()=>{
            if (this.ctx.duration > 0 && this.ctx.buffered.length > 0){
                this.load_progress = this.ctx.buffered.end(this.ctx.buffered.length-1) / this.ctx.duration
            }
        ...
        this.ctx.addEventListener('timeupdate', ()=>{
            if (this.ctx.duration) this.play_progress = this.ctx.currentTime/this.ctx.duration
    ...
    shift(offset, auto_play=false) {
        ...
        // shuffle
            ...
            let shuffled_ptr = this.shuffled_list.findIndex(idx=>(idx === this.song_ptr))
            shuffled_ptr += offset
